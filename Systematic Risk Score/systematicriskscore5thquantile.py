...
def plot_systemic_risk(df_risk):
    ...
    plt.plot(df_plot.index, df_plot['ScoreNorm'],  marker='o', color='teal',
             label='Systemic Risk Index',  linewidth=1, markersize=3)
    plt.plot(df_plot.index, df_plot['DensityNorm'],marker='o', color='darkorange',
             label='Network Density [%]', linewidth=1, markersize=3)

    # Title
    plt.title("Systemic Risk Score & Network Density (Rolling 364-day Window)")
    ...
    # No forced ylim so it can go above 100 if needed
...
if __name__ == "__main__":
    ...
    path_marketcap = "/content/data_wide_marketcap.csv"
    path_close     = "/content/data_wide_close.csv"
    ...
    df_risk.to_csv("systemic_risk_output.csv")

    # 5) Plot from 2021 onward
    plot_systemic_risk(df_risk)

    print("\nDone. Results in 'systemic_risk_output.csv'. Plot shown above.")
