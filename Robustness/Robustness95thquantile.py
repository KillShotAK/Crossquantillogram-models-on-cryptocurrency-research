...
def cross_quantilogram(y1, y2, tau=0.95, max_lag=10):
    ...
    Modified to use 95th quantile (tau=0.95) instead of 5th.
    """
    results = {}
...
def stationary_bootstrap_test(y1, y2, tau=0.95, max_lag=10, B=100, alpha=0.05):
    """
...
def build_adjacency_matrix(returns_window, tau=0.95, alpha=0.05, max_lag=10, B=100):
    """
...
def rolling_systemic_risk(df_ret, size_vec, window_size=364, step=30,
                         tau=0.95, alpha=0.05, max_lag=10, B=100):
    """
...
def run_window_analysis(df_returns, bank_size_vec, window_size):
    ...
    df_risk = rolling_systemic_risk(
        ...
        step=30,
        tau=0.95,  # Modified to use 95th quantile instead of 5th
        alpha=0.05,
        max_lag=10,
        B=100
    )
...
def plot_systemic_risk_comparison(df_300, df_364, df_400):
    ...
    plt.title('Comparison of Normalized Systemic Risk Index (95th Quantile) Across Different Rolling Windows')
...
if __name__ == "__main__":
    ...
    print("\nComputing systemic risk for different window sizes (95th quantile)...")
    df_risk_300 = run_window_analysis(df_returns, bank_size_vec, 300)
    df_risk_300.to_csv("systemic_risk_300_q95.csv")

    df_risk_364 = run_window_analysis(df_returns, bank_size_vec, 364)
    df_risk_364.to_csv("systemic_risk_364_q95.csv")

    df_risk_400 = run_window_analysis(df_returns, bank_size_vec, 400)
    df_risk_400.to_csv("systemic_risk_400_q95.csv")

    # Create and save comparison plot
    print("\nGenerating comparison plot...")
    fig = plot_systemic_risk_comparison(df_risk_300, df_risk_364, df_risk_400)
    fig.savefig("systemic_risk_comparison_q95.png", bbox_inches='tight', dpi=300)
    plt.close()

    print("\nDone. Results saved in CSV files and plot saved as 'systemic_risk_comparison_q95.png'")