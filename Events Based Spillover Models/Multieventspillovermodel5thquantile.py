...
logging.basicConfig(
    ...
    filename='crypto_analysis_5th_quantile.log'
)
...
def compute_event_adjacency(
    ...
    quantile: float = 0.05,  # 5th quantile (changed from 0.95)
    max_lag: int = 5,
    n_bootstrap: int = 100,
...
def plot_network_on_axis(
    ...
    for (u, v, d) in edges_data:
        ...
        nx.draw_networkx_edges(
            ...
            edgelist=[(u, v)],
            width=edge_width,
            # Changed edge color to red for negative tail risk (5th quantile)
            edge_color='red',
            alpha=0.6,
            arrows=True,
            arrowsize=15,
    ...
    # 6. Title
    # Modified title to reflect 5th quantile analysis
    ax.set_title(f"{title} (5th Quantile)", fontsize=10)
    ax.set_axis_off()
...
def main():
    ...
    Main flow:
      ...
      3) Compute single adjacency with 5th quantile (changed from 95th)
      4) Threshold top-100
      5) Create network
    ...
    try:
        ...
        for event_name, (start_date, end_date) in events.items():
            ...
            adj_matrix, w_matrix = compute_event_adjacency(
                ...
                quantile=0.05,  # Changed from 0.95 to 0.05 (5th quantile)
                max_lag=5,
                n_bootstrap=100,
                threshold=0.05
            )
        ...
        # Changed filename to indicate 5th quantile analysis
        outpath = 'crypto_spillover_results/multi_event_networks_5th_quantile.png'
