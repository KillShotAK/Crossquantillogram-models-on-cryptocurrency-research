...
def cross_quantilogram(y1, y2, tau=0.05, max_lag=10):
    ...
    Now defaults to tau=0.05 for 5th quantile analysis (lower tail risk).
    """
    y1 = np.array(y1)
    y2 = np.array(y2)
    T  = len(y1)
...
def stationary_bootstrap_test(y1, y2, tau=0.05, max_lag=10, B=200, block_prob=0.1):
    """
...
def build_adjacency_matrix(returns_data, tau=0.05, max_lag=10,
...
def rolling_net_effect(returns_data, window_size=250, step=50,
                       tau=0.05, max_lag=10, alpha=0.05,
                       B=50, block_prob=0.1):
    """
...
def main():
    ...
    computes adjacency, net effect for 5th quantile, applies dual_scale, and plots.
    """
    # ----------------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------------
    # 6) Full Sample Analysis (NOW FOR 5TH QUANTILE - LOWER TAIL RISK)
    # ----------------------------------------------------------------------------
    print("=== Full Sample Adjacency Calculation (5th Quantile) ===")
    print(f"Analyzing {len(df_returns.columns)} tokens at 5th quantile (lower tail risk)...")
    
    A_full = build_adjacency_matrix(
        df_returns,
        tau=0.05,        # Changed from 0.95 to 0.05 for 5th quantile analysis
        max_lag=10,
        alpha=0.05,
        B=50,
        block_prob=0.1
    ...
    df_net_effect_full = net_effect_series.to_frame(name="FullSample_5th")
    ...
    # ----------------------------------------------------------------------------
    # 7) Plotting (Updated titles to reflect 5th quantile)
    # ----------------------------------------------------------------------------
    ...
    fig1, ax1 = plot_net_effect(
        ...
        "Net Effect (Transmission - Reception)\n(Full Sample, 5th Quantile, Dual-Scaled)",
        filename="full_sample_net_effect_5th.png"
    )
    
    # Plot the full adjacency matrix
    fig_adj, ax_adj = plot_adjacency_matrix(
        A_full_scaled,
        "Adjacency Matrix (Full Sample, 5th Quantile, Dual-Scaled)",
        filename="full_sample_adjacency_matrix_5th.png"
    )

    # ----------------------------------------------------------------------------
    # 8) Rolling Analysis (Optional, can be slow with large datasets)
    # ----------------------------------------------------------------------------
    print("=== Rolling Net Effect Calculation (5th Quantile) ===")
    ...
    df_net_effect_roll = rolling_net_effect(
        ...
        tau=0.05,         # Changed from 0.95 to 0.05 - 5% quantile (lower tail risk)
    ...
    if not df_net_effect_roll.empty:
        ...
        fig2, ax2 = plot_net_effect(
            ...
            f"Rolling Net Effect - 5th Quantile ({df_net_effect_roll.columns[0].strftime('%Y-%m-%d')})",
            filename="first_window_net_effect_5th.png"
        )
        
        # Plot the full rolling visualization (with all windows)
        fig_roll, ax_roll = plot_rolling_net_effect(
            df_net_effect_roll_scaled,
            "Rolling Net Effect (All Windows, 5th Quantile, Dual-Scaled)",
            filename="rolling_net_effect_matrix_5th.png"
        )
        
        print(f"Created {len(df_net_effect_roll.columns)} rolling windows for 5th quantile analysis")
    else:
        print("No rolling windows were calculated. Check your data length.")

    print("5th quantile analysis complete!")
